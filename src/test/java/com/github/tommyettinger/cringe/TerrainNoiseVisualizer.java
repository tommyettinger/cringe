...
/**
 * Shows levels of noise as different colors, drawn from the libGDX {@link Colors} class.
 */
public class TerrainNoiseVisualizer extends ApplicationAdapter {
...
//    private final ContinuousNoise noise = new ContinuousNoise(new FoamNoise(), 1337, 0.04f, ContinuousNoise.FBM, 3);
...
//    private static final int width = 256, height = 256;
    ...
    private static final float DESERT = Color.GOLDENROD.toFloatBits();
    private static final float WATER = Color.ROYAL.toFloatBits();
    private static final float GRASS = Color.FOREST.toFloatBits();
    private static final float STONE = Color.DARK_GRAY.toFloatBits();
    /**
     * This set of terrain thresholds was given as an example by tridvajedan on Discord.
     * I wanted to see how it would look with different noise.
     */
    private FloatToFloatFunction prepare = (floatTile) -> {
        if(floatTile < -0.9f) return DESERT;
        else if(floatTile < -0.45f) return WATER;
        else if(floatTile < -0.35f) return DESERT;
        else if(floatTile < -0.14f) return GRASS;
        else if(floatTile < 0f)   return STONE;
        else if(floatTile < 0.2f) return GRASS;
        else if(floatTile < 0.5f) return STONE;
        else return GRASS;
    };
...
//                }
    ...
    public static void main(String[] arg) {
        ...
        new Lwjgl3Application(new TerrainNoiseVisualizer(), config);
    }
}
