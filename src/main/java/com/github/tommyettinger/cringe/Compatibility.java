 ...
 * This overlaps a little with {@link com.badlogic.gdx.utils.NumberUtils}. It includes methods like
 * {@link #floatToReversedIntBits(float)} (which makes converting from an OpenGL packed ABGR float
 * to an RGBA8888 int very easy) and {@link #doubleToMixedIntBits(double)} (which is useful when
 * implementing hashCode() for double values). Everything's optimized for GWT, which is important
 * because some core JDK methods like {@link Float#floatToIntBits(float)} are quite slow on GWT.
 * This makes heavy use of JS typed arrays to accomplish its conversions; these are widespread even
 * on mobile browsers, and are very convenient for this sort of code (in some ways, they're a
 * better fit for this sort of bit-level operation in JavaScript than anything Java provides).
 * <br>
 * Additionally, this provides wrappers around some code that has substantially faster or better
 * implementations in the current JS language than what GWT provides out-of-the-box. This includes
 * {@link #countLeadingZeros(int)}, which calls {@link Integer#numberOfLeadingZeros(int)} on most
 * platforms, but uses JavaScript's {@code Math.clz32()} on GWT. Similarly, basic int-by-int
 * multiplication can lose precision in JS, but can't on any other platforms; JavaScript provides
 * {@code Math.imul()} for this purpose (which cannot lose precision), and so we have
 * {@link #imul(int, int)} here that calls the JS code on GWT. There's also
 * {@link #lowestOneBit(long)}, which fixes a behavior bug in GWT, and {@link #getExponent(float)}.
 *
